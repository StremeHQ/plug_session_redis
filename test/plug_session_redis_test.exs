defmodule PlugSessionRedisTest do
  ...
  test "fetches an existing session" do
    :poolboy.transaction(
      @poolboy_name,
      &:redo.cmd(&1, ["SET", "123", BinaryEncoder.encode!(%{key: :value})])
    )

    conn =
      conn(:get, "/foo") |> Plug.Conn.put_resp_cookie("_my_app_key", "123") |> SampleApp.call([])
  ...
  test "updates an existing session" do
    :poolboy.transaction(
      @poolboy_name,
      &:redo.cmd(&1, ["SET", "456", BinaryEncoder.encode!(%{key: :value})])
    )

    conn =
      conn(:get, "/add") |> Plug.Conn.put_resp_cookie("_my_app_key", "456") |> SampleApp.call([])
  ...
  test "deletes an existing session" do
    :poolboy.transaction(
      @poolboy_name,
      &:redo.cmd(&1, ["SET", "789", BinaryEncoder.encode!(%{key: :value})])
    )

    conn =
      conn(:get, "/destroy")
      |> Plug.Conn.put_resp_cookie("_my_app_key", "789")
      |> SampleApp.call([])

    # executes before send which removes session
